...
//		GroundMovementMode = MovementMode;
		...
		UpdateComponentRotation(false, false);
		AdjustFloorHeight();
	...
	// Scoped updates can improve performance of multiple MoveComponent calls
	{
		...
		if (!bHandledNetUpdate || !bNetworkSkipProxyPredictionOnNetUpdate || !NinjaCharacterMovementCVars::NetEnableSkipProxyPredictionOnNetUpdate)
		{
			...
			if (IsMovingOnGround() || MovementMode == MOVE_Falling)
			{
				...
				else if (IsMovingOnGround() || (!Gravity.IsZero() && ((Velocity | Gravity) * -1.0f) <= 0.0f))
				{
...
void UNinjaCharacterMovementComponent::MaybeUpdateBasedMovement(float DeltaSeconds)
{
	...
	UpdateComponentRotation(bAlwaysRotateAroundCenter || !bMovingOnGround,
...
void UNinjaCharacterMovementComponent::PhysFalling(float deltaTime, int32 Iterations)
{
	...
	while (RemainingTime >= MIN_TICK_TIME && Iterations < MaxSimulationIterations)
	{
		...
		if (bNotifyApex && ((Velocity | GravityDir) * -1.0f) <= 0.0f)
		{
...
void UNinjaCharacterMovementComponent::PhysicsVolumeChanged(class APhysicsVolume* NewVolume)
{
		...
		NewVolume == GetWorld()->GetDefaultPhysicsVolume())
	{
		...
		SetFixedGravityDirection(FVector(0.0f, 0.0f, -1.0f));
	}
...
void UNinjaCharacterMovementComponent::OnClientCorrectionReceived(FNetworkPredictionData_Client_Character& ClientData, float TimeStamp, FVector NewLocation, FVector NewVelocity, UPrimitiveComponent* NewBase, FName NewBaseBoneName, bool bHasBase, bool bBaseRelativePosition, uint8 ServerMovementMode)
{
	...
	if (NinjaCharacterMovementCVars::NetShowCorrections != 0)
	{
		...
		if (!LocDiff.IsNearlyZero())
		{
			...
			DrawDebugCapsule(GetWorld(), ClientLocAtCorrectedMove, CharacterOwner->GetSimpleCollisionHalfHeight(), CharacterOwner->GetSimpleCollisionRadius(), UpdatedComponent->GetComponentQuat(), FColor(255, 100, 100), true, DebugLifetime);
		...
		else
		{
			...
			DrawDebugCapsule(GetWorld(), NewLocation, CharacterOwner->GetSimpleCollisionHalfHeight(), CharacterOwner->GetSimpleCollisionRadius(), UpdatedComponent->GetComponentQuat(), FColor(255, 255, 100), true, DebugLifetime);
		}
	}
#endif //!UE_BUILD_SHIPPING
...
void UNinjaCharacterMovementComponent::ClientSetFixedGravityDirection_Implementation(const FVector& NewFixedGravityDirection)
{
...
void UNinjaCharacterMovementComponent::ClientSetSplineTangentGravityDirection_Implementation(AActor* NewGravityActor)
{
...
void UNinjaCharacterMovementComponent::ClientSetPointGravityDirection_Implementation(const FVector& NewGravityPoint)
{
	if (GravityDirectionMode == ENinjaGravityDirectionMode::Point &&
		GravityVectorA == NewGravityPoint)
	{
...
void UNinjaCharacterMovementComponent::ClientSetPointGravityDirectionFromActor_Implementation(AActor* NewGravityActor)
{
	if (GravityDirectionMode == ENinjaGravityDirectionMode::Point &&
		GravityActor == NewGravityActor)
	{
...
void UNinjaCharacterMovementComponent::ClientSetLineGravityDirection_Implementation(const FVector& NewGravityLineStart, const FVector& NewGravityLineEnd)
{
...
void UNinjaCharacterMovementComponent::ClientSetSegmentGravityDirection_Implementation(const FVector& NewGravitySegmentStart, const FVector& NewGravitySegmentEnd)
{
...
void UNinjaCharacterMovementComponent::ClientSetSplineGravityDirection_Implementation(AActor* NewGravityActor)
{
...
void UNinjaCharacterMovementComponent::ClientSetPlaneGravityDirection_Implementation(const FVector& NewGravityPlaneBase, const FVector& NewGravityPlaneNormal)
{
...
void UNinjaCharacterMovementComponent::ClientSetSplinePlaneGravityDirection_Implementation(AActor* NewGravityActor)
{
...
void UNinjaCharacterMovementComponent::ClientSetBoxGravityDirection_Implementation(const FVector& NewGravityBoxOrigin, const FVector& NewGravityBoxExtent)
{
	if (GravityDirectionMode == ENinjaGravityDirectionMode::Box &&
		GravityVectorA == NewGravityBoxOrigin && GravityVectorB == NewGravityBoxExtent)
	{
...
void UNinjaCharacterMovementComponent::ClientSetBoxGravityDirectionFromActor_Implementation(AActor* NewGravityActor)
{
	if (GravityDirectionMode == ENinjaGravityDirectionMode::Box && GravityActor == NewGravityActor)
	{
		return;
...
void UNinjaCharacterMovementComponent::ClientSetCollisionGravityDirection_Implementation(AActor* NewGravityActor)
{
...
void UNinjaCharacterMovementComponent::SetAlignGravityToBase(bool bNewAlignGravityToBase)
{
	...
	if (ShouldReplicateGravity())
	{
		...
		{
			ClientDisableAlignGravityToBase();
		}
		else
		{
			ClientEnableAlignGravityToBase();
		}
	}
}

void UNinjaCharacterMovementComponent::ClientEnableAlignGravityToBase_Implementation()
{
	bAlignGravityToBase = true;
}

void UNinjaCharacterMovementComponent::ClientDisableAlignGravityToBase_Implementation()
{
	bAlignGravityToBase = false;
}
...
void UNinjaCharacterMovementComponent::ReplicateGravityToClients()
{
	...
	if (bDirtyGravityDirection)
	{
		...
		switch (GravityDirectionMode)
		{
			case ENinjaGravityDirectionMode::Fixed:
			{
				ClientSetFixedGravityDirection(GravityVectorA);
				break;
			}

			case ENinjaGravityDirectionMode::SplineTangent:
			{
				ClientSetSplineTangentGravityDirection(GravityActor);
				break;
			}

			case ENinjaGravityDirectionMode::Point:
			{
				ClientSetPointGravityDirection(GravityVectorA);
				break;
			}

			case ENinjaGravityDirectionMode::Line:
			{
				ClientSetLineGravityDirection(GravityVectorA, GravityVectorB);
				break;
			}

			case ENinjaGravityDirectionMode::Segment:
			{
				ClientSetSegmentGravityDirection(GravityVectorA, GravityVectorB);
				break;
			}

			case ENinjaGravityDirectionMode::Spline:
			{
				ClientSetSplineGravityDirection(GravityActor);
				break;
			}

			case ENinjaGravityDirectionMode::Plane:
			{
				ClientSetPlaneGravityDirection(GravityVectorA, GravityVectorB);
				break;
			}

			case ENinjaGravityDirectionMode::SplinePlane:
			{
				ClientSetSplinePlaneGravityDirection(GravityActor);
				break;
			}

			case ENinjaGravityDirectionMode::Box:
			{
				ClientSetBoxGravityDirection(GravityVectorA, GravityVectorB);
				break;
			}

			case ENinjaGravityDirectionMode::Collision:
			{
				ClientSetCollisionGravityDirection(GravityActor);
				break;
			}
		}
...
FRotator UNinjaCharacterMovementComponent::ConstrainComponentRotation(const FRotator& Rotation) const
{
	...
	return FNinjaMath::MakeFromZQuat(CapsuleUp, Rotation.Quaternion()).Rotator();
}
...
void UNinjaCharacterMovementComponent::SetAlignComponentToFloor(bool bNewAlignComponentToFloor)
{
	...
	if (ShouldReplicateGravity())
	{
		...
		{
			ClientDisableAlignComponentToFloor();
		}
		else
		{
			ClientEnableAlignComponentToFloor();
		}
	}
}

void UNinjaCharacterMovementComponent::ClientEnableAlignComponentToFloor_Implementation()
{
	bAlignComponentToFloor = true;
}

void UNinjaCharacterMovementComponent::ClientDisableAlignComponentToFloor_Implementation()
{
	bAlignComponentToFloor = false;
}
...
void UNinjaCharacterMovementComponent::SetAlignComponentToGravity(bool bNewAlignComponentToGravity)
{
	...
	if (ShouldReplicateGravity())
	{
		...
		{
			ClientDisableAlignComponentToGravity();
		}
		else
		{
			ClientEnableAlignComponentToGravity();
		}
	}
}

void UNinjaCharacterMovementComponent::ClientEnableAlignComponentToGravity_Implementation()
{
	bAlignComponentToGravity = true;
}

void UNinjaCharacterMovementComponent::ClientDisableAlignComponentToGravity_Implementation()
{
	bAlignComponentToGravity = false;
}
...
void UNinjaCharacterMovementComponent::UpdateComponentRotation(bool bRotateAroundCenter, bool bRotateVelocity)
{
	if (!HasValidData())
	{
		return;
	...
	const FVector CurrentCapsuleUp = GetComponentAxisZ();
	...
	if (!bRotateAroundCenter)
	{
		...
		UpdatedComponent->MoveComponent(Delta, NewRotation, true);
	}
	else
	{
		// Not using MoveUpdatedComponent to bypass constraints
		UpdatedComponent->MoveComponent(FVector::ZeroVector, NewRotation, true);
	}

	if (bRotateVelocity && !Velocity.IsZero())
	{
		// Modify Velocity direction to prevent losing speed on rotation change
		Velocity = FQuat::FindBetweenNormals(CurrentCapsuleUp, DesiredCapsuleUp).RotateVector(Velocity);
	}
}
